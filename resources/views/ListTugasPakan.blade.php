...
@section('content')
    ...
    <body>
        ...
        <div class=" mx-[16px] mt-[90px] md:mt-[0px]  md:mx-[60px] md:pt-[213px] ">
            ...
            <div class="flex ">
                @isset($kandang)
                    <form action="{{ route('tugas.storePakan', ['season' => request()->segment(1), 'kandang' => $kandang]) }}" method="POST" enctype="multipart/form-data">

                @endisset
                <form action="{{ route('tugas.storePakans', ['season' => request()->segment(1)]) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf

                    <button
                        class="bg-[#047857] text-white text-[18px] font-semibold text-left pl-[12px] w-[123px] md:w-[147px] h-[45px]  md:h-[54px] shadow-md mb-6 rounded-r-[10px]">Pakan</button>


                        
                    @isset($kandangs)
                        <div class="mb-[15px]">
                            <label class="block mb-[5px] text-[18px] font-semibold" id="pilih-kandang">Pilih Kandang</label>    
                            <div class="grid grid-cols-2">
                                @foreach ($kandangs as $kandang)
                                <div>
                                    <input type="checkbox" name="kandang[]" id="pilih-kandang-checkbox-{{ $kandang->id }}" value="{{ $kandang->id }}">
                                    <label for="pilih-kandang-checkbox-{{ $kandang->id }}">{{ $kandang->name }}</label>
                                </div>
                                @endforeach
                            </div>
                            <button type="button" class="text-[#047857] border p-2 rounded mt-2" onclick="pilihSemuaKandang()">Pilih Semua</button>
                            <button type="button" class="ml-2 border p-2 rounded" onclick="batalPilihSemuaKandang()">Batal</button>
                        </div>
                    @endisset
                    <div class="mb-[15px]">
                    ...
                    <div class="mb-[15px]">
                        ...
                        <select name="status" value="{{ old('status', '') }}"
                            ...
                            <option value="Sudah dikerjakan" {{ old('status') == 'Sudah dikerjakan' ? 'selected' : '' }}>Sudah Dikerjakan</option>
