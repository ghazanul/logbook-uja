...
@section('content')
    ...
    <body>
        <div class="mx-[16px] mt-[90px] md:mt-[0px]  md:mx-[60px] md:pt-[213px]">
            ...
            <div class="flex ">
                @isset($kandang)
                    <form action="{{ route('tugas.storeTelur', ['season' => $season, 'kandang' => $kandang]) }}" method="POST" enctype="multipart/form-data">
                @endisset
                <form action="{{ route('tugas.storeTelurs', ['season' => $season]) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf

                    <button
                        class="bg-[#047857] text-white text-[18px] font-semibold text-left pl-[12px] w-[147px] h-[45px] md:h-[54px] shadow-md mb-6 rounded-r-[10px]">Telur</button>
